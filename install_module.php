...
HTML;
	...
	if ($_REQUEST['action'] == "install") {
		...
		} else if ( $config['version_id'] == "11.1" ) {
...
HTML;
		mainTable_foot();
	} else if ($_REQUEST['action'] == "manual") {
		if ( function_exists( 'file_get_contents' ) ) { $xmlstr = file_get_contents( ROOT_DIR. "/install/xml/" . $module['file'] ); }
		else { $h = fopen( ROOT_DIR. "/install/xml/" . $module['file'], 'r' ); $xmlstr = fread( $h, filesize( ROOT_DIR. "/install/xml/" . $module['file'] ) ); fclose( $h ); }
...
HTML;
			foreach ( $operations as $opid => $operation ) {
				$process = "";
				if ( isset( $operation['operation']->index ) ) { $process .= $lang['m39'] . $operation['operation']->index; }
				if ( isset( $operation['operation']->offset ) ) { $process .= $lang['m40'] . $operation['operation']->offset . $lang['m41']; }
				$pro_lang = array( "before" => $lang['m42'], "replace" => $lang['m43'], "after" => $lang['m44'], "ibefore" => $lang['m45'], "iafter" => $lang['m46'], );
